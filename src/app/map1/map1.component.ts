// map1 is a component of the parent app.
...
//  April 2020, the clicked-aiports are to be added to a routeList
// inside this map1 component.

import { Component, OnInit, Input } from '@angular/core';
import { NgModule } from '@angular/core';
import { MouseEvent } from '@agm/core';
// apr 2020 gotta make a smaller json file for quicker testing
import AirportJson from '../airportData5010Mi.json';
//import { parseIntAutoRadix } from '@angular/common/src/i18n/format_number';
import { Map1AgmMarker } from './map1AgmMarker'; 

// these were comments since 2019
//import * as MarkerClusterer from "@google/markerclusterer";
// npm install js-marker-clusterer @agm/js-marker-clusterer --save
//import { AgmJsMarkerClustererModule, ClusterManager } from '@agm/js-marker-clusterer'; 
// apr 2020 comment out ng module?
////@NgModule({
...
export class Map1Component implements OnInit {
  @Input() inputMsg1FromParent = '?' 
  @Input() inputRouteOnOffFromParent = '?' 
// use weird angular @Input when I want a component to read a var that is
// controlled by the parent.   
// @Input is in this here child component.
// There is a weird tie between this child, and the parent.
// The weird tie is in parent -->  app.component.html   
// app.component.html has [inputMsg1FromParent] = "msg1"
// this binds the child @Input var (inputMsg1FromParent) to the parent var (msg1)
// someday i gotta come up with a short naming convention for weird angular input
// maybe @Input() waiVarname.
// if that aint weird enough, look at @Output with event emitter.
// ==============================================================
//  Google Cloud & Google Map notes 
...
///////////////////////////////////////////////////////////////////
 ...
 lat: number = 43.2000
 lng: number = -85.4000
//  funkyAgmMarker1: Map1AgmMarker;
 markerArray1: Map1AgmMarker[] = [] ;
 routeList = '?'
...
ngOnInit() {
  ...
  // this.funkyAgmMarker1 = new Map1AgmMarker;
...
onMarkerClick(mParmIn: Map1AgmMarker) {
  // duz he want to display airport info ?
  // or duz he want to add to the route?
  // look at inputRouteOnOffFromParent to determine this.

 // alert('running onMarkerClick. ' )
 // alert( 'inputMsg1FromParent is like: ' + this.inputMsg1FromParent)
 // alert('is routing on or off ? ' + this.inputRouteOnOffFromParent)
   
  if (this.inputRouteOnOffFromParent == 'On') {
    //  push the airport object into the markersArray.
    this.markersArray.push(mParmIn)
    this.buildRouteList()
    // eventually, stick the route  list on an ag-grid
  }  // end if routing On
      
  //let i = (this.markersArray.length-1)
  //(this.markersArray[i].locationId)
...
onMarkerRightClick(mParmIn: Map1AgmMarker) {
  // if routing is On, then he is trying to remove this marker
  // from the route list.
  // re-create the markersArray, filtering out the rightclicked marker
  this.markersArray = this.markersArray
    .filter(a => a.locationId != mParmIn.locationId);
  this.buildRouteList()  
} // end onMarkerRightClick

checkIncludeMarker(oneDog,moo){
  // return true if locationId should be kept. false to remove.   
    if (oneDog.locationId == moo.locationId ) {
      return false ;
    } else {
      return true ; }
} // checkIncludeMarker
...
buildRouteList(){
  //alert('running buildRouteList')
  this.routeList = ''
  for (let i=0; i<this.markersArray.length; i++) { 
      this.routeList = 
        this.routeList + this.markersArray[i].locationId + "|"
  }
    alert('routeList:' + this.routeList)
}
...
buildAirportList() {
  ...
  //clean way to filter, but weirdly, each array element is not a parm

  var icono =   {
    // maybe use a different icon for big airports.
    // duz 5010 file tell us class b , class c ?
    url: this.iconGrnDot ,
  ...
  for (let i = 0; i < this.airportJson2.length; i++) {
    ...
    this.airportJson2[i].MagneticVariation,
    ) ;
  }
...
//this.markersArray.push(marker2); 
//add this marker to the array of markers so that we can later clear them all
...
//alert ('addToRouteMK');
//alert(this.position);  
 //needs fixing when we put more markers than just home marker is on the map.
